...
class History:
    ...
    def total(self) -> int:
        ...
        return self.state.timeline.total(frame.total if frame is not None else 1)
    ...
    def live_index(self) -> int:
        ...
        if frame is not None:
            return frame.index
        entries = self.state.timeline.entries
    ...
    def displayed_surface(self) -> pygame.Surface | None:
        ...
        return self.state.timeline.preview_surface() or self.app.canvas.frame_surface
    ...
    def refresh_label(self) -> None:
        ...
        app = self.app
        e = self.state.timeline.preview_entry()
        if e is not None:
            text = f"{e.label} {e.index}/{e.total}"
        else:
            frame = self.state.worker.latest_frame() if self.state.worker is not None else None
    ...
    def sync_preview_prompts(self) -> None:
        ...
        app = self.app
        entry = self.state.timeline.preview_entry()
    ...
    def refresh_preview_state(self) -> None:
        ...
        self.refresh_label()
        self.app.bottom_bar.gate_play_for_preview(self.state.timeline.preview_index is not None)
    ...
    def revert(self) -> None:
        ...
        if self.state.timeline.preview_index is None:
            return
        entry = self.state.timeline.entries[self.state.timeline.preview_index]
        if (
            self.state.worker is None
        ):  # loaded-session history (no latent) -> img2img from its preview
            self._revert_loaded(entry)
            return
        # Adopt the checkpoint's prompts as the live set, then branch from it.
        self.state.prompts = [PromptRow(t, w, m) for t, w, m in entry.prompts]
        ...
        entry.config.apply_to(self.state.session.config)
        self.cancel_guidance_checkpoint()
        app.sidebar.refresh_advanced_widgets(app)
        self.state.worker.seek(self.state.timeline.preview_index)
        ...
        app.canvas.paint.reset_overlays(self.state.worker.paint_applied_count)
    ...
    def keyboard_revert(self) -> None:
        ...
        app = self.app
        if self.state.worker is None or app.running or not self.state.timeline.entries:
            return
        self.state.timeline.begin_undo()
        self.revert()
    ...
    def sync(self) -> None:
        ...
        if tl.sync(self.state.worker.get_history() if self.state.worker is not None else None):
